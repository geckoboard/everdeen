...
RSpec.describe Everdeen::Server, type: :unit do
  ...
  describe '#initialize' do
    it 'assigns proxy_port and control port and store_requests' do
      expect(subject.proxy_port).to eq 1090
      expect(subject.control_port).to eq 1091
      expect(subject.store_requests).to eq true
    end
  end

  describe '#start' do
    let(:server) { Everdeen::Server.new(proxy_port: 8080, control_port: 8081, store_requests: true) }
    let(:host) { '127.0.0.1' }
